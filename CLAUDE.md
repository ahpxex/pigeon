...
**Tab 活动监测**（`Tabs/TabActivityMonitor.swift`）：每秒采样各 tab 视口（`viewportText()`，只读一屏；`screenText()` 遍历全部 scrollback，别用它采样），区分**自发输出**与打键回显（变化距 `lastUserInputAt` <2s 算回显，永不点亮 spinner）。busy（侧边栏图标换 spinner）**只属于 TUI 会话**：Enter 落在"非 shell prompt"状态（`needsConfirmQuit` true，即打给正在跑的前台 TUI——判定利用 `.pigeonSurfaceDidSubmit` 通知先于按键到达终端的时序）才即亮；shell prompt 上的 Enter（跑命令，无论快慢）永不亮。会话内 agent 停顿后恢复输出需连续 2 个自发样本重亮（孤立重绘不闪）；shift+Enter 是 TUI 换行不算提交。熄灭：输出停止 1.5s 即灭（提交后 3s 首 token 宽限），回到 shell prompt 立即灭并结束会话。⚠️ 用户若手改 confirm-close-surface 为 always/false 会失真（该 API 折算了这个配置）。提交时抓视口尾部快照进 `recentSubmits`（通知先于按键到达终端，输入框里还是用户输入，shell/TUI 通吃）。busy 结束时用户没在看（app active + 选中 + 窗口 key）→ `hasUnread`，label 末尾圆点，查看即清。`workSeconds` 累计用户交互后的自发输出秒数。driver /state 带 `busy`/`unread`。
