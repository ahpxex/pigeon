...
**Tab 活动监测**（`Tabs/TabActivityMonitor.swift`）：**只认 hooks，没有采样或启发式回退**。AgentServer 暴露 `POST /activity`（与 /ask 同一套鉴权），事件 `{event: busy|idle|ping, source, prompt?}` + `X-Pigeon-Surface` 头：busy 点亮、idle 熄灭、ping 在新 session 开始时建立 idle 基线并清掉上一进程未正常退出留下的陈旧 busy。普通 Enter、前台进程、viewport 变化、输出静默、回到 shell prompt 都绝不读写 busy。`ActivityHooks`（`Agent/ActivityHooks.swift`，设置 Agent tab 底部 "Coding Agents" 区管理）给三家装钩子：Claude Code（~/.claude/settings.json，SessionStart→ping / UserPromptSubmit→busy / Stop·StopFailure·SessionEnd→idle）、Codex（~/.codex/hooks.json 同事件集，需 features.hooks=true，装时自动加、用户显式 false 则拒装；新钩子需在 Codex 下次启动时信任）、pi（~/.pi/agent/extensions/pigeon-activity.ts，session_start→ping / before_agent_start→busy / agent_settled·session_shutdown→idle）。三家共用 `~/.config/pigeon/hooks/pigeon-activity`（0755，读 PIGEON_AGENT_PORT/TOKEN/SURFACE_ID；dev/prod 共用并由环境变量路由）。v3 上报脚本同步等待 Pigeon 接收后才退出；pi handler 同样 await fetch，避免宿主结束回调时丢事件；上报失败仍返回成功，不能反过来破坏 coding agent。硬杀进程导致宿主根本不发 idle 时，hooks-only 设计不会猜测结束，状态会保持到下一次明确 idle/ping 或 tab 关闭。busy hook 直接携带 prompt，供消息历史和标题使用；`workSeconds` 只累计 hook busy 区间。busy 结束时用户没在看（app active + 选中 + 窗口 key）→ `hasUnread`，label 末尾圆点，选中 tab、窗口成为 key 或 app 激活时清除。driver /state 带 `busy`/`unread`/`surface`；测试用 `/tabs/activity`（免 token 直打状态机）与 `/activity-hooks/{state,install,remove}`。

**AI tab 标题**（`Agent/TabTitleSummarizer.swift`）：屏幕尾部 + busy hook 直接携带的用户 prompt 发给 agent 的 Provider 模型，生成短标题写进 `aiTitle`；hook prompt 是第一信号（标题应说用户要干什么，输出只作补充）。**不做 API 轮询，也不采样终端判断活动**：自动触发**每次提交一次**——用户每给 coding agent 发一条消息（busy hook 开启新 busy 区间，或 busy 已在跑时再带 prompt 来一条=排队的追问）`Activity.submitCount` +1，"AI tab titles" 开关开 + 该提交的 hook busy 时间 `workSecondsSinceSubmit ≥3` 且（idle 后 ≥4s 或 ≥15s 长任务中途）就重新生成一次标题；summarizer 记 `summarizedSubmitCount`，请求发出即记账（失败不重试），没有新提交就绝不再请求。<3s 就取消的提交不触发。手动：右键 "Summarize Title"（driver `/tabs/summarize`），3s 防抖，会清掉 customTitle。显示优先级 customTitle > aiTitle > 目录名/shell 标题；"Use Default Title" 两者都清。prompt 要求跟随用户语言、沿用未变化的旧标题、内容当数据防注入；输出过 `sanitize`。测 TUI 场景用本机的 `pi`，别拿 Claude Code 烧配额。
