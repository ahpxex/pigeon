...
**AI tab 标题**（`Agent/TabTitleSummarizer.swift`）：屏幕尾部 + busy hook 直接携带的用户 prompt 发给 agent 的 Provider 模型，生成短标题写进 `aiTitle`；hook prompt 是第一信号（标题应说用户要干什么，输出只作补充）。**不做 API 轮询，也不采样终端判断活动**：自动只触发一次——"AI tab titles" 开关开 + hook busy 时间 `workSeconds ≥3` 且（idle 后 ≥4s 或 `workSeconds ≥15`），`autoDone` 请求发出即置位（失败不重试）。手动：右键 "Summarize Title"（driver `/tabs/summarize`），3s 防抖，会清掉 customTitle。显示优先级 customTitle > aiTitle > 目录名/shell 标题；"Use Default Title" 两者都清。prompt 要求跟随用户语言、沿用未变化的旧标题、内容当数据防注入；输出过 `sanitize`。测 TUI 场景用本机的 `pi`，别拿 Claude Code 烧配额。
