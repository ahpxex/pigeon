...
**Tab 活动监测**（`Tabs/TabActivityMonitor.swift`）：每秒采样每个 tab 的可见视口（`SurfaceView.viewportText()`，VIEWPORT 空间 read_text，只读一屏，别用会遍历全部 scrollback 的 `screenText()` 干这事），纯本地零网络。核心区分"**自发输出**"与"打键回显"：屏幕变化发生在最近一次用户输入（`SurfaceView.lastUserInputAt`，keyDown/performKeyEquivalent/driver sendKey·sendText 都会打点）2s 内算回显，否则算工具自己在产出。连续 2 个自发变化样本 → `TerminalTab.isBusy = true`（侧边栏该 tab 的 OpenMoji 图标换成 spinner，Claude Code/Codex/构建跑的时候一眼可见），3s 无任何变化 → 回落。同时累计 `workSeconds`（用户交互之后的自发输出秒数），供标题总结判定"真实工作会话"。driver /state 的 tab JSON 带 `busy` 字段。

**AI tab 标题**（`Agent/TabTitleSummarizer.swift`）：把终端屏幕尾部（40 行/4KB）发给已配置的 Provider 模型，生成 ≤4 词/12 字的标题写进 `TerminalTab.aiTitle`。为跑 coding agent 的 tab 而生：目录名区分不了三个都在 `~/Code/x` 里的 Claude Code。**刻意不做 API 轮询**，只有两个触发点：(1) 自动一次——General 设置的 "AI tab titles" 开关打开时，基于活动监测的工作日志判定"真实工作会话发生了"：`workSeconds ≥3`（用户交互后工具自发输出了至少 3 秒——⚠️ 别退回"字符数阈值"方案：Claude Code/Codex 这类 TUI 一启动就画满屏，字符阈值会在用户什么都没干时就误触发）且（屏幕静默 ≥4s 即任务收尾，或 `workSeconds ≥15` 即长任务跑着也直接总结不再干等），调用一次模型，此后不再自动重总结（`autoDone` 在请求发出时置位，失败不会变成重试循环）；(2) 手动——tab 右键 "Summarize Title"（driver 对应 `/tabs/summarize`、`pigeonctl summarize <id>`），随时可刷新，3s 防抖，手动会先清掉 customTitle（用户点了就是要新的）。显示优先级 customTitle > aiTitle > 目录名/shell 标题（与标签风格正交，任何风格下都生效）；右键 "Use Default Title" 同时清 custom 和 AI 标题。prompt 要求沿用上次标题、按屏幕主导语言输出、把屏幕内容当数据防注入；输出过 `sanitize`（取首行、剥引号、折叠空白、40 字上限按词边界截断）。复用 agent 的 Provider/key/model（AgentSettings），无独立配置。
