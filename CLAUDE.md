...
- 用户在设置的 Agent Tab 配置 AI Provider（内置 Anthropic/OpenAI/DeepSeek/OpenRouter + 自定义 OpenAI 兼容端点），模型可选、API key 按 Provider 独立存 `~/.config/pigeon/credentials.json`（0600）—— 这套配置就是给内置 Agent 用的。⚠️ 别改回 Keychain：Keychain ACL 绑定代码签名身份，对自编译 app 意味着重编译/更新后反复弹授权框（已踩过）。内置 Provider 的 UUID 是写死的常量，key 按 UUID 索引，UUID 不稳定会让 key 变孤儿。
...
**Tab 活动监测**（`Tabs/TabActivityMonitor.swift`）：双层机制 —— **钩子层（权威）** + **采样层（兑底）**。**钩子层**：AgentServer 暴露 `POST /activity`（与 /ask 同一套鉴权），事件 `{event: busy|idle|ping, source}` + `X-Pigeon-Surface` 头。`ActivityHooks`（`Agent/ActivityHooks.swift`，设置 Agent tab 底部 "Coding Agents" 区管理）给三家装钩子：Claude Code（~/.claude/settings.json，SessionStart→ping / UserPromptSubmit→busy / Stop·StopFailure·SessionEnd→idle）、Codex（~/.codex/hooks.json 同事件集，需 features.hooks=true，装时自动加、用户显式 false 则拒装；钩子需在 Codex 下次启动时信任）、pi（~/.pi/agent/extensions/pigeon-activity.ts，session_start/before_agent_start/agent_settled）。三家都调同一个上报脚本 `~/.config/pigeon/hooks/pigeon-activity`（0755，读 PIGEON_AGENT_PORT/TOKEN/SURFACE_ID 环境变量 → 后台 curl；dev/prod 共用，环境变量自动路由到正确实例；终端外 no-op）。收到过钩子事件的 surface 置 `hooksPresent`（tab 生命周期内 sticky）：**抑制一切启发式点亮**（Enter 即亮、视口变化重点），busy/idle 由事件驱动；采样层的熄灭路径（输出停 1.5s、回 prompt）保持作为丢失 idle 事件的兑底。**采样层**（无钩子的 TUI 兑底）：每秒采样各 tab 视口（`viewportText()`，只读一屏；`screenText()` 遍历全部 scrollback，别用它采样），区分**自发输出**与打键回显（变化距 `lastUserInputAt` <2s 算回显，永不点亮 spinner；⚠️ 鼠标不更新 lastUserInputAt，这是已知失真源）。busy（侧边栏图标换 spinner）**只属于 TUI 会话**：Enter 落在"非 shell prompt"状态（`needsConfirmQuit` true，即打给正在跑的前台 TUI——判定利用 `.pigeonSurfaceDidSubmit` 通知先于按键到达终端的时序）才即亮；shell prompt 上的 Enter（跑命令，无论快慢）永不亮。会话内 agent 停顿后恢复输出需连续 2 个自发样本重亮（孤立重绘不闪）；shift+Enter 是 TUI 换行不算提交。熄灭：输出停止 1.5s 即灭（提交后 3s 首 token 宽限），回到 shell prompt 立即灭并结束会话。⚠️ 用户若手改 confirm-close-surface 为 always/false 会失真（该 API 折算了这个配置）。提交时抓视口尾部快照进 `recentSubmits`（通知先于按键到达终端，输入框里还是用户输入，shell/TUI 通吃）。busy 结束时用户没在看（app active + 选中 + 窗口 key）→ `hasUnread`，label 末尾圆点，查看即清。`workSeconds` 累计用户交互后的自发输出秒数。driver /state 带 `busy`/`unread`/`surface`；测试用 `/tabs/activity`（免 token 直打状态机）与 `/activity-hooks/{state,install,remove}`。

**AI tab 标题**（`Agent/TabTitleSummarizer.swift`）：屏幕尾部 + 提交快照发给 agent 的 Provider 模型，生成短标题写进 `aiTitle`；提交快照是第一信号（标题应说用户要干什么，输出只作补充）。**不做 API 轮询**：自动只触发一次——"AI tab titles" 开关开 + `workSeconds ≥3` 且（静默 ≥4s 或 `workSeconds ≥15`），`autoDone` 请求发出即置位（失败不重试）。⚠️ 别退回"字符数阈值"触发：Claude Code/Codex 这类 TUI 一启动就画满屏，会在用户什么都没干时误触发。手动：右键 "Summarize Title"（driver `/tabs/summarize`），3s 防抖，会清掉 customTitle。显示优先级 customTitle > aiTitle > 目录名/shell 标题；"Use Default Title" 两者都清。prompt 要求跟随用户语言、沿用未变化的旧标题、内容当数据防注入；输出过 `sanitize`。测 TUI 场景用本机的 `pi`，别拿 Claude Code 烧配额。
...
已实现：垂直 Tab 侧边栏（多 tab、切换保活、关闭、拖拽排序（组内）、右键重命名（customTitle 覆盖 shell 标题）、cmd+T/W、cmd+1-9 走 ghostty 键位；拖拽调宽 160-420、拖到 <120 或 ⌘B 折叠，状态持久化在 UserDefaults，`WorkspaceState`）、**cmd+F 搜索回滚缓冲区**（纯前端实现，零内核新 API：`SurfaceSearch.swift` 用 SCREEN 空间 rectangle read_text 分块 dump——rectangle 模式一屏幕行一输出行，绕开软换行合并；宽字符列映射用 wcwidth 前缀和（进程需先 `setlocale(LC_CTYPE,"UTF-8")`，GUI 进程默认 C locale 会把 CJK 算成窄字符）；跳转用 `binding_action` 的 scroll_to_bottom 锚定 + scroll_page_lines 精确滚行；高亮是 SwiftUI Canvas overlay，位置来自 read_text 返回的视口 offset（row*cols+col）+ tl_px_x + config window-padding-y；搜索框是 NSTextField 包装（SwiftUI FocusState 抢不过终端 NSView 的 first responder），Esc/Enter/⇧Enter 走 `doCommandBy`，⇧ 判定读 `NSApp.currentEvent`（类属性 modifierFlags 是物理键盘状态，合成事件测不到）。⚠️ 依赖 `patches/ghostty-screen-point-clamp.patch`：上游 Point.pin 把 SCREEN 空间 y 也钳到视口行数，scrollback 寻址全塌缩；补丁只对 active/viewport 钳 y。⚠️ 选区 coord 用 EXACT——TOP_LEFT/BOTTOM_RIGHT 返回的 pin x 不在行边缘，rectangle dump 会塌成一列。已知限制：跨软换行边界的匹配搜不到；输出滚动时全局匹配列表会陈旧（视口高亮每 400ms 自刷新）；匹配上限 2000）、窗口配色与终端主题统一（hiddenTitleBar 全铺背景色）、分组（TabGroup：折叠/改名/删除，右键 Move to Group，侧边栏空白处单击新建组并行内命名（空名=取消），拖拽 tab 到组头或组区域直接入组（高亮提示），组内拖拽排序，用户建的空组保留到手动删除；**按目录自动分组**（General 开关，默认开）：跟随 OSC 7 pwd 变化，cd 进目录即加入/创建以目录名命名的组（`TabGroup.autoPath` 按路径匹配，重名先复用），cd 回 home 出组，auto 组空了即清；只对真实 cwd *变化*反应，所以手动分组保持到下次 cd（`TabManager.autoGroup`））、OpenMoji tab 图标（56 个精选 128px PNG 分五类打进 bundle，新 tab 随机分配且避开在用图标，右键 Change Icon 弹分类网格选择器；OpenMoji CC BY-SA 4.0 需保留署名）、驱动服务与 pigeonctl、键盘（含基本 IME preedit）、鼠标、剪贴板、标题（默认显示 OSC 7 上报的目录名，custom rename 优先）、按住 ⌘ 显示 tab 跳转序号（cmd+1-9 按侧边栏视觉顺序）、光标形状、bell、URL 打开、Ghostty 配置加载。
...
- **永远不许直接关掉正在运行的生产版 Pigeon**（kill / pkill / kill -9 / AppleScript quit，任何形式都不行）。生产版里跑着用户的活跃会话（pi/Claude Code 等），强杀 = 毁掉用户正在干的事 —— 已踩过一次（kill -9 杀掉带着 4 个 pi 会话的生产版，被用户当场骂）。替换 /Applications/Pigeon.app 的正确姿势：先构建好新 Release 放在 build/ 里，然后**告诉用户，等用户自己退出生产版**，确认进程不在了再 cp 替换。优雅退出的尝试（AppleScript quit + 点确认框）在生产版上也不许自作主张地升级成强杀，失败就停下来问用户。（2026-08-22）
