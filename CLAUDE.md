...
**Tab 活动监测**（`Tabs/TabActivityMonitor.swift`）：每秒采样各 tab 视口（`viewportText()`，只读一屏；`screenText()` 遍历全部 scrollback，别用它采样），区分**自发输出**与打键回显（变化距 `lastUserInputAt` <2s 算回显，永不点亮 spinner）。busy（侧边栏图标换 spinner）：bare Enter 提交即亮（`.pigeonSurfaceDidSubmit` 通知，shift+Enter 是 TUI 换行不算）；无提交时连续 2 个自发样本才亮（单次孤立重绘不闪）；距最后自发输出/提交 3s 无更新即灭。提交时抓视口尾部快照进 `recentSubmits`（通知先于按键到达终端，输入框里还是用户输入，shell/TUI 通吃）。busy 结束时用户没在看（app active + 选中 + 窗口 key）→ `hasUnread`，label 末尾圆点，查看即清。`workSeconds` 累计用户交互后的自发输出秒数。driver /state 带 `busy`/`unread`。
